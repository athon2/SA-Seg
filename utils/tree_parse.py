...
def tree_parse_modified(mha_dir):
    # mha_dir = "" #ground truth or predictions
    file_list = os.listdir(mha_dir)
    # file_list.sort()
    from random import shuffle
    shuffle(file_list)
    for case in file_list:
        print(case)
        label_array, label_img = read_mha(os.path.join(mha_dir, case,'airway_segments_label_lc.nii.gz'))
        save_name_parse = os.path.join(mha_dir, case, "parse.nii.gz")
        save_name_skel = os.path.join(mha_dir, case, "skel.nii.gz")
        if os.path.exists(save_name_parse) and os.path.exists(save_name_skel):
            continue
        label = (label_array>0).astype(np.uint8)
        label = ndimage.binary_fill_holes(label)
        label = large_connected_domain(label)
        skeleton = skeletonize_3d(label)
        skeleton_parse, cd, num = skeleton_parsing(skeleton)
        ...
        # save_name_parse = os.path.join(mha_dir, case, "parse.nii.gz")
        # save_name_skel = os.path.join(mha_dir, case, "skel.nii.gz")
        # save_name_label = os.path.join(mha_dir, case, "label.mha")
        # print(skeleton.shape)
        # print(tree_parsing.shape)
        # img_nii = nibabel.Nifti1Image(img, np.eye(4))
        # nibabel.save(img_nii, save_name_img)
        # parse_nii = nibabel.Nifti1Image(tree_parsing, np.eye(4))
        # nibabel.save(parse_nii, save_name_parse)
        ...
        # label_nii = nibabel.Nifti1Image(label, np.eye(4))
        # nibabel.save(label_nii, save_name_label)
        npy2mha(tree_parsing, save_name_parse, label_img.GetSpacing(), label_img.GetOrigin())
        npy2mha(skeleton, save_name_skel, label_img.GetSpacing(), label_img.GetOrigin())

        # save_name_pm = os.path.join(mha_dir, case, "parent.npy")
        # save_name_cm = os.path.join(mha_dir, case, "children.npy")
        # np.save(save_name_pm, parent_map)   
        # np.save(save_name_cm, children_map)

if __name__=='__main__':
    # mha_dir = 'E:/workspace/airway/CN_label/surgery_before/refined_mha'
    # mha_dir = "/root/workspace/airway_reconstruction/data/original_data_private"
    mha_dir = "/data/segmental_bronchus/airway_data/new_original_data"
    tree_parse_modified(mha_dir)
